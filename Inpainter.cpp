...
void Inpainter::inpaint(){
	...
	//	cout << "first" << totaltime1 << endl;
	//	cout << "second" << totaltime2 << endl;
...
void Inpainter::computeBestPatch(){
	...
	for (int x = 0; x <= workImage.cols - width; x += halfPatchWidth){
		for (int y = 0; y <= workImage.rows - height; y += halfPatchWidth)
		if (sourceRegion.at<uchar>(y, x) == 1)
		{
			patchError = 0;
