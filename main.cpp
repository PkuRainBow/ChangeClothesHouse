...
//存储hairmarker中标记的头发点（bgdPxls）到XML，并标记到&mask上
	...
	try
	{
		if (save){	//保存头发点
			...
			for (vector<Point>::iterator it = hairmarker.fgdPxls.begin(); it != hairmarker.fgdPxls.end(); it++){
				...
				circle(mask, *it, 1, GC_FGD, -1);	//标记mask上头发的位置为前景
			}
			myDocument->SaveFile();
		}
		else{	//不保存头发点
			...
			for (vector<Point>::iterator it = hairmarker.bgdPxls.begin(); it != hairmarker.bgdPxls.end(); it++){
				circle(mask, *it, 1, GC_BGD, -1);	//标记mask上头发的位置为前景
			}
		}
	}
	catch (string &e){
...
void findSquare_txt(vector<Point> &iCord, vector<Point> facePoints)
{
	...
	if (facePoints.size() != 0)
	{
		c = 10 * (facePoints[23].x - facePoints[24].x);	//23点对应68点中的27点,24点对应68点中的33点
...
//Grabcut，将comMask（4值：0/1/2/3）二值化得到binMask（2值：0/1），0 2 -> 0; 1 3 -> 1;
...
void adjShoulder(vector<Point> &iCord, vector<Point> &facePoints){
	//若肩膀线过于靠内: iCord[2]位于facePoints[3]、facePoints[4]的切线内; iCord[num - 3]位于facePoints[13]、facePoints[12]的切线内
	//调整iCord[2]或iCord[num - 3]到对应脸部点的切线上。
...
void imageMatting_txt_model(Mat &imgOld, const Mat &maskOld, vector<Point> &facePoints, vector<Point> &iCord, string outFileName){
	Mat mask, mask0, image;		//mask: Grabcut迭代过程中的遮罩; mask0: 根据脸、肩点画出的遮罩; image: mask0遮在原图上得到的图片，用于Grabcut算法抠图
	cvtColor(maskOld, mask0, CV_BGR2GRAY);
	imgOld.copyTo(image, mask0);
	mask.create(image.size(), CV_8UC1);

	Mat res, binmask, showmask, showbinmask;	//res: 记录Grabcut结果图像; binmask: 将mask取值0、1化; showmask、showbinmask: 直观显示mask、binmask
	Mat bgdModel, fgdModel;

	int numi = iCord.size();
	Point p1(iCord[0]);			//左下
	Point p2(iCord[numi - 1]);	//左上
	Point p3(iCord[numi - 3]);	//右下
	Point p4(iCord[numi - 2]);	//右上
	int topY = max(0, p2.y);
	...
	//根据mask0的左下、左上、右下、右上4个点，确定两条边线，向内收缩作为背景线
	int leftEdgeX = p2.x + double(facePoints[0].y - p2.y) / (p1.y - p2.y) * (p1.x - p2.x);	//facePoint[0]投射到mask0左边线上的X坐标
	int rightEdgeX = p4.x - double(facePoints[16].y - p4.y) / (p3.y - p4.y) * (p4.x - p3.x);	//facePoint[16]投射到mask0右边线上的X坐标
	...
	//-----将抠出的mask放大----------------------------------------------------------------------------------------------------------------------------------
	//将binmask的height以上的部分截出，放大，再居中放回原来binmask中相同的位置。
	getBinMask(mask, binmask);
	int height = max(iCord[0].y, iCord[numi - 3].y);
	double scale = 1.2;
	Mat adjmask = binmask(Rect(0, 0, binmask.cols, height));	//adjmask取binmask中height以上的部分
	Size dsize(adjmask.cols * scale, adjmask.rows * scale);
	Mat tmp, newbin;
	Mat showadj, shownew, showbin, tmpbin, showtmp;
	resize(adjmask, tmp, dsize, 0, 0, CV_INTER_AREA);	//放大dsize倍存入tmp中
	adjmask = tmp(Rect((tmp.cols - adjmask.cols) / 2, (tmp.rows - adjmask.rows) / 2, adjmask.cols, adjmask.rows));	//从tmp中居中取出原adjmask大小的部分
	newbin = adjmask | binmask(Rect(0, 0, binmask.cols, height));	//放大后的adjmask与原来的binmask做or，防止放大后的adjmask中黑色部分覆盖到binmask中本该是白色的位置
	newbin.copyTo(binmask(Rect(0, 0, binmask.cols, height)));	//将局部放大后的mask贴回原处
	//showbin = binmask.clone();	//------------可能涉及线程操作问题，加此行防止后续copyto操作出问题-------------
	newbin.release();
	binmask.copyTo(newbin, mask0);	//用mask0规约肩线和下巴
	//shownew = newbin.clone();	//------------可能涉及线程操作问题，加此行防止后续copyto操作出问题-------------
	binmask = newbin;
	//--------------------------------------------------------------------------------------------------------------------------------------------------
...
void imageMatting_txt_hair(const Mat &img_big, const Mat &img_small, const Mat &mask_small, vector<Point> &facePoints, vector<Point> &iCord, string outFileName){
	int Thickness = 6;
	Mat mask, mask0, image;		//mask: Grabcut迭代过程中的遮罩; mask0: 根据脸、肩点画出的遮罩;
	Mat res, binmask, showmask, showbinmask;	//res: 记录Grabcut结果图像; binmask: 将mask取值0、1化; showmask、showbinmask: 直观显示mask、binmask
	cvtColor(mask_small, mask0, CV_BGR2GRAY);
	img_small.copyTo(image);	//保留肩膀以下
	//img_small.copyTo(image, mask0);	//不保留肩膀以下
...
void drawShoulder_txt(string filePre, string picInfo, string outPre)
{
	...
	// 读取picture information
	ifstream fin(picInfo, ios::_Nocreate);

	ofstream fout;

	fout.open(outPre + "height.txt");	//记录shoulder的最低高度

	if (!fin)
		return;

	while (!fin.eof()){
		//读取图片、key points、pose信息
		string fileName;
		vector<Point> facePoints(27);	//0~16, 19, 24, 36, 39, 42, 45, 27, 33, 48, 54	大图
		vector<Point> facePoints_small(27);	//0~16, 19, 24, 36, 39, 42, 45, 27, 33, 48, 54	小图
		vector<Point> shoulderPoints(6);	//左~右	大图
		vector<Point> shoulderPoints_small(6);	//左~右	小图
		...
		/*
		----------图片缩放，picWidth_big：输出mask、图片的大小； picWidth_small：抠图图片的大小------------
		*/
		...
		// find the upper square 根据肩膀&脸部特征点位置，确定mask向上延伸的方向
		...
		fillPoly(mask_big, polys, Scalar(255, 255, 255));	//poly为mask的轮廓点
